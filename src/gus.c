...
extern void GUSSetVolume(word Volume);
...
//music_sdata buffer that holds our pattern data
dword offset = 0;
word offset1 = 0;               // offset of note in pattern data buffer.

// PROTOTYPES
extern void interrupt (*old_time_handler)(void); 
...
void GUSSetVolume(word Volume){
	asm{
	mov  dx,Set_volume
	mov  ax, 9
	out  dx, ax
	inc  dx
	mov  ax, Volume// now reference the right GUS volume.
	out  dx, ax
	}
}
...
//69.666
void interrupt mod_player_fast() {
...
void StopMOD() {
	...
	setvect(0x1C,old_time_handler);       // put the old timer back on its vector
...
void PlayMOD(byte sp, byte bp) {
	...
	setvect(0x1C,  mod_player_fast);		//interrupt 1C not available on NEC 9800-series PCs.
...
int LoadMOD(char *filename) {
	...
	_fmemset(music_sdata, 0, size);                 // wipes buffer clean
	...
	for (pattcount=0; pattcount <= MOD.numpats; pattcount++) {
		for (row=0; row<64; row++) {   			// loop down through 64 notes.
			for (count=0; count<channels; count++) {
				// point our little note structure to music_sdata
				current = (struct Note *)(music_sdata + offset);
				...
				vol = part[3]<<1;
				if (vol > 64) vol = 64;
				current->eparm = GUSvol[vol];       // parameter
...
void Init_GUS(){
	...
	if (ptr != NULL){
		Base = ((ptr[1]-48)*0x10)+0x200;   	// else grab the hex base address
		GUSReset(4);             			// initialize GUS with 4 voices (44.1khz)
